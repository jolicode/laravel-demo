    ...
    <div class="container flex flex-wrap items-center justify-between mx-auto p-4">
        ...
        <div class="hidden w-full md:block md:w-auto" id="navbar-default">
            <ul class="font-medium flex flex-col p-4 md:p-0 mt-4 border border-gray-100 rounded-lg bg-gray-50 md:flex-row md:space-x-8 rtl:space-x-reverse md:mt-0 md:border-0 md:bg-white dark:bg-gray-800 md:dark:bg-gray-900 dark:border-gray-700">
                ...
                @else
                    <li class="flex text-{{ $route == 'login' ? 'blue-500' : 'white' }} md:text-{{ $route == 'login' ? 'blue-500' : 'white' }} dark:text-{{ $route == 'login' ? 'blue-500' : 'white' }}">
                        <i class="fa fa-sign-in"></i>
                        <a href="{{ route('login') }}" class="block py-2 px-3 bg-blue-700 rounded md:bg-transparent md:p-0">Login</a>
                    </li>
                @endif
